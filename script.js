...
// --- Interactive Voice Assistant (Speech Recognition & Synthesis) ---
// recognition and voiceEnabled are declared at the top for global scope consistency
...
if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
    ...
    recognition.onresult = (event) => {
        ...
        // Process voice command directly (works even without the chatbox)
        const lowerMsg = transcript.toLowerCase();
        let bestMatch = null;
        let maxScore = 0;

        // CIVIC_KNOWLEDGE may not be defined yet at this point, so guard it
        if (typeof CIVIC_KNOWLEDGE !== 'undefined') {
        ...
        const response = (TRANSLATIONS[currentLang] && TRANSLATIONS[currentLang][responseKey]) 
            ? TRANSLATIONS[currentLang][responseKey] 
            : "I'm listening! Ask me about voting, booths, or your next steps.";
        
        // Speak the response
        const speechText = response.replace(/<[^>]*>?/gm, '');
        speak(speechText);
        
        // Execute navigation action if matched
        if (bestMatch && bestMatch.action) {
            setTimeout(() => bestMatch.action(), 1000);
        }
        
        // Also populate the chatbox if it exists (dashboard is visible)
...
// --- Steel-Plated Voice Manager ---
function safeStartRecognition() {
    if (!voiceEnabled || !recognition || window.speechSynthesis.speaking) return;
    try {
        recognition.start();
        console.log("🎙️ Vantage: Microphone is LIVE");
    } catch (e) {
...
function stopRecognition() {
    if (recognition) {
        ...
        console.log("🛑 Vantage: Microphone is OFF");
    }
}

// recognition.onend is now handled in the initial setup above

/**
 * Toggles the AI Voice Assistant state — Siri/Alexa style.
 */
function toggleVoiceAssistant() {
    ...
    const bubbleText = document.getElementById('voice-transcript-text');
    ...
    if (voiceEnabled) {
        ...
        // Show transcript bubble with greeting
        bubbleText.textContent = TRANSLATIONS[currentLang]['greet'];
        bubble.className = 'voice-visible';
        statusLabel.textContent = 'Listening...';
        statusLabel.className = 'voice-visible';
        
        showToast('🎙️ Voice Assistant Active — Speak naturally!');
        window.speechSynthesis.cancel();
        speak(TRANSLATIONS[currentLang]['greet']);
    } else {
        btn.classList.remove('active', 'listening', 'speaking');
        icon.textContent = 'mic_none';
        
        bubble.className = 'voice-hidden';
        statusLabel.className = 'voice-hidden';
...
/**
 * Updates the voice orb's visual state.
...
function setVoiceOrbState(state) {
    ...
    if (state === 'listening') {
        ...
        if (statusLabel) { statusLabel.textContent = 'Listening...'; statusLabel.className = 'voice-visible'; }
    } else if (state === 'speaking') {
        btn.classList.add('speaking');
        if (icon) icon.textContent = 'record_voice_over';
        if (statusLabel) { statusLabel.textContent = 'Speaking...'; statusLabel.className = 'voice-visible'; }
    } else {
        if (icon) icon.textContent = voiceEnabled ? 'mic' : 'mic_none';
        if (statusLabel && voiceEnabled) { statusLabel.textContent = 'Tap to speak'; statusLabel.className = 'voice-visible'; }
    }
}

/**
 * Shows a message in the transcript bubble.
 */
function showTranscript(text, isUser) {
    ...
    bubbleText.innerHTML = isUser 
        ? `<span style="color:var(--text-muted); font-size:0.7rem; display:block; margin-bottom:4px;">You said:</span>${text}`
        : text;
    bubble.className = 'voice-visible';
    
    // Auto-hide after 6 seconds if not speaking
    clearTimeout(window._transcriptTimeout);
    window._transcriptTimeout = setTimeout(() => {
        if (!window.speechSynthesis.speaking && voiceEnabled) {
            bubble.className = 'voice-hidden';
        }
    }, 6000);
}
...
function speak(text) {
    ...
    const targetLang = (currentLang === 'hi') ? 'hi' : (currentLang === 'gu') ? 'gu' : 'en';
    
    let langVoice = voices.find(v => v.lang.startsWith(targetLang) && v.name.includes('Google')) ||
                    voices.find(v => v.lang.startsWith(targetLang));
    
    if (!langVoice && targetLang === 'gu') {
        langVoice = voices.find(v => v.lang.startsWith('hi') && v.name.includes('Google')) ||
                    voices.find(v => v.lang.startsWith('hi'));
    }
    
    if (langVoice) currentUtterance.voice = langVoice;
    currentUtterance.lang = langVoice ? langVoice.lang : (targetLang === 'gu' ? 'gu-IN' : 'hi-IN');
...
// Pre-fetch voices to prevent initial silent fail
